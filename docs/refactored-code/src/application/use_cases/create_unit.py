...
class CreateUnitUseCase:
    ...
    def execute(
        self, tracking_id: TrackingId, initial_status: UnitStatus = UnitStatus.CREATED
    ) -> dict:
        """
        ...
        if existing_unit:
            logger.warning("Unidad ya existe", tracking_id=str(tracking_id))
        ...
        initial_checkpoint_data = CheckpointData(
            status=initial_status, timestamp=unit.created_at
        )
