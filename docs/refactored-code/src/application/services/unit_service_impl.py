...
class UnitServiceImpl(UnitService):
    ...
    def create_unit(self, tracking_id: TrackingId) -> Unit:
        ...
        logger.info("Creando nueva unidad", tracking_id=str(tracking_id))
        ...
        logger.info(
            "Unidad creada exitosamente", tracking_id=str(tracking_id), unit_id=unit.id
        )

        return unit

    def add_checkpoint(
        self, tracking_id: TrackingId, checkpoint_data: CheckpointData
    ) -> Unit:
