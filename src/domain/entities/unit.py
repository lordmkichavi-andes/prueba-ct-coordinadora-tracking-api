...
class Unit:
    ...
    @classmethod
    def create(
        cls, tracking_id: TrackingId, initial_status: UnitStatus = UnitStatus.CREATED
    ) -> "Unit":
        """Factory method para crear una nueva unidad"""
        now = datetime.utcnow()
        initial_checkpoint = CheckpointData(status=initial_status, timestamp=now)
    ...
    def add_checkpoint(self, checkpoint_data: CheckpointData) -> None:
        ...
        if (
            self.checkpoints
            and checkpoint_data.timestamp <= self.checkpoints[-1].timestamp
        ):
            raise ValueError(
                "El timestamp del nuevo checkpoint debe ser posterior al último"
            )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "delivery_time": (
                self.get_delivery_time().isoformat()
                if self.get_delivery_time()
                and hasattr(self.get_delivery_time(), "isoformat")
                else None
            ),
        }
