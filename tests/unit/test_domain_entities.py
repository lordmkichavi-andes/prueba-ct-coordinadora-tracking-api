...
class TestTrackingId:
    ...
    def test_short_tracking_id_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Tracking ID debe tener al menos 3 caracteres"):
            TrackingId("AB")
    
    def test_long_tracking_id_raises_error(self):
        """Test que tracking ID muy largo lanza error"""
        long_id = "A" * 51
        with pytest.raises(ValueError, match="Tracking ID no puede exceder 50 caracteres"):
            TrackingId(long_id)
    
    def test_invalid_characters_raises_error(self):
        """Test que caracteres inválidos lanzan error"""
        with pytest.raises(ValueError, match="Tracking ID solo puede contener letras, números, guiones y guiones bajos"):
            TrackingId("ABC@123")
...
class TestCheckpointData:
    ...
    def test_long_location_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Location no puede exceder 200 caracteres"):
            CheckpointData(
                status=UnitStatus.PICKED_UP,
                timestamp=datetime.utcnow(),
...
class TestUnit:
    ...
    def test_checkpoint_timestamp_validation(self):
        ...
        with pytest.raises(ValueError, match="El timestamp del nuevo checkpoint debe ser posterior al último"):
    ...
    def test_unit_is_delivered(self):
        ...
        checkpoints = [
            ...
            CheckpointData(status=UnitStatus.OUT_FOR_DELIVERY, timestamp=datetime.utcnow()),
